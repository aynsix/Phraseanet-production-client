...
//                _openPreview(
     ...
     * @param reload
     */
    function _openOnlyPreview(event, env, pos, contId, reload) {
        if (contId === undefined) {
            contId = '';
        }
        var roll = 0;
        ...
        prevAjax = $.ajax({
            ...
            success: function (data) {
                ...
                $('.video-subtitle-wrapper').empty().append($template.get(0));
                if ($(`#${customId}`).length > 0) {
                    activeThumbnailFrame = new pym.Parent(
                        customId,
                        data.record.preview.url
                ...
                $('.video-subtitle-wrapper .thumb_wrapper')
                    .width('100%')
                    .height('100%')
                    .image_enhance({ zoomable: true });
                resizeVideoPreview();
